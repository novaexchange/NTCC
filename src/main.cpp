...
#if defined(NDEBUG)
# error "NeptuneClassic cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0d8f1b0608fc538d1216c572ee9eb088333b59b70dc32cff43e69dd99740db18");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // NeptuneClassic: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "NeptuneClassic Signed Message:\n";
...
{
    // NeptuneClassic: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 230000 blocks
    //nSubsidy >>= (nHeight / 230000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NeptuneClassic: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NeptuneClassic: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1483717505, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 06Jan2017 Chancellor on brink of second bailout for banks";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04244519388d4f8f71c41e82d1922875d723b0e235a05ef120d6d662bc15b5edea8293593b9d0b9554d890da6a00f260bd9e6b7c12e92f5c0ec052e7f61beb4a87") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1483717505;
        ...
        assert(block.hashMerkleRoot == uint256("0x2a795401d1815aa09a5bb43fc671f4e811e5d1e1c3e449d81090021fb1c3d752"));
...
unsigned char pchMessageStart[4] = { 0x00, 0xd8, 0xc7, 0x6a }; // NeptuneClassic: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NeptuneClassicMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NeptuneClassicMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NeptuneClassicMiner(CWallet *pwallet)
{
    printf("NeptuneClassicMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("neptuneclassic-miner");
    ...
    try { loop {
        ...
        printf("Running NeptuneClassicMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NeptuneClassicMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NeptuneClassicMiner, pwallet));
}
